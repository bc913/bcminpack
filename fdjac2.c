...
__cminpack_attr__
int __cminpack_func__(fdjac2bc)(__cminpack_decl_fcn_bc_mn__ void *p, int m, int n, real *x, 
	const real *fvec, const int *iuser, const real *user, real *fjac, int ldfjac,
	real epsfcn, real *wa)
{
    /* Local variables */
    real h;
...
/*     epsmch is the machine precision. */
	...
	x[j] = temp + h;
        /* the last parameter of fcn_mn() is set to 2 to differentiate
           ...
           the Jacobian (see fcn() in examples/lmfdrv.c, and how njev
           is used to compute the number of Jacobian evaluations) */
	iflag = fcn_bc_mn(p, m, n, x, wa, 2, iuser, user);
	if (iflag < 0) {
            return iflag;
	}
	x[j] = temp;
