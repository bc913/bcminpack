...
__minpack_attr__
void __minpack_func__(lmdifbc)(__minpack_decl_fcn_bc_mn__ const int *m, const int *n, real *x, 
	real *fvec, const int* iuser, const real *user, 
    const real *ftol, const real *xtol, const real *
	gtol, const int *maxfev, const real *epsfcn, real *diag, const int *
	mode, const real *factor, const int *nprint, int *info, int *
	nfev, real *fjac, const int *ldfjac, int *ipvt, real *
	qtf, real *wa1, real *wa2, real *wa3, real *
...
/*     and calculate its norm. */
    ...
    iflag = 1;
    fcn_bc_mn(m, n, &x[1], &fvec[1], &iflag, iuser, user);
    *nfev = 1;
    if (iflag < 0) {
	goto L300;
    }
...
/*        calculate the jacobian matrix. */
    ...
    iflag = 2;
    __minpack_func__(fdjac2bc)(__minpack_param_fcn_bc_mn__ m, n, &x[1], &fvec[1], iuser, user, &fjac[fjac_offset], ldfjac, &
	    iflag, epsfcn, &wa4[1]);
    *nfev += *n;
    if (iflag < 0) {
	goto L300;
...
/*           evaluate the function at x + p and calculate its norm. */
    ...
    iflag = 1;
    fcn_bc_mn(m, n, &wa2[1], &wa4[1], &iflag, iuser, user);
    ++(*nfev);
    if (iflag < 0) {
	goto L300;
    }
