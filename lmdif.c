...
__cminpack_attr__
int __cminpack_func__(lmdifbc)(__cminpack_decl_fcn_bc_mn__ void *p, int m, int n, real *x, 
	real *fvec, const int* iuser, const real *user, 
    real ftol, real xtol, real
	gtol, int maxfev, real epsfcn, real *diag, int
	mode, real factor, int nprint, int *
	nfev, real *fjac, int ldfjac, int *ipvt, real *
	qtf, real *wa1, real *wa2, real *wa3, real *
...
/*     and calculate its norm. */
    ...
    iflag = fcn_bc_mn(p, m, n, x, fvec, 1, iuser, user);
    *nfev = 1;
    if (iflag < 0) {
	goto TERMINATE;
    }
...
/*        calculate the jacobian matrix. */
        ...
        iflag = __cminpack_func__(fdjac2bc)(__cminpack_param_fcn_bc_mn__ p, m, n, x, fvec, iuser, user, fjac, ldfjac,
                       epsfcn, wa4);
        *nfev += n;
        if (iflag < 0) {
            goto TERMINATE;
...
/*        if requested, call fcn to enable printing of iterates. */
        ...
        if (nprint > 0) {
            iflag = 0;
            if ((iter - 1) % nprint == 0) {
                iflag = fcn_bc_mn(p, m, n, x, fvec, 0, iuser, user);
            }
            if (iflag < 0) {
                goto TERMINATE;
            }
...
/*           evaluate the function at x + p and calculate its norm. */
            ...
            iflag = fcn_bc_mn(p, m, n, wa2, wa4, 1, iuser, user);
            ++(*nfev);
            if (iflag < 0) {
                goto TERMINATE;
            }
...
/*     termination, either normal or user imposed. */
    ...
    if (nprint > 0) {
	fcn_bc_mn(p, m, n, x, fvec, 0, iuser, user);
    }
    return info;
