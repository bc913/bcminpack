...
__minpack_attr__
void __minpack_func__(fdjac2bc)(__minpack_decl_fcn_bc_mn__ const int *m, const int *n, real *x, 
	const real *fvec, const int *iuser, const real *user,
...
/*     epsmch is the machine precision. */
	...
	x[j] = temp + h__;
	fcn_bc_mn(m, n, &x[1], &wa[1], iflag, iuser, user);
	if (*iflag < 0) {
	    /* goto L30; */
            return;
	}
